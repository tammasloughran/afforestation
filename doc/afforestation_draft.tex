...
Combined with differences in grid resolution, this results in a variety of changes global tree cover for each model (see Figure \ref{fig:tree_area}).
As an example, by the end of the century ACCESS-ESM1-5 has a forest expansion of 1.59 million km² and agricultural abandonment of 1.11 million km².
...
\begin{figure}[H]
    ...
    \caption{Tree cover area change between 2015 to 2100 in the forestation scenario for each model. (Several models are missing the treeFrac variable.)}
...
Correlations were done using the Spearman rank correlation and the significance for each ensemble member was tested at the 5\% level.
...
\begin{itemize}
    ...
    \item CanESM has net deforestation by the end of the century, but increases in cVeg. It also has no significant change in global temperature by the end of the century.
    \item ACCESS-ESM is roughly in the middle of the model spread. More similar to MPI-ESM and UKESM than other models.
    \item For vegetation related pools (cLand and cVeg) models tend to diminish in productivity by the end of the century. Models  either maintain a draw-down potential of ~20-30PgC by 2100, or the gains from the middle of the century have been lost by the end of the century.
...
\begin{subfigure}[b]{0.4\linewidth}
    ...
    \end{subfigure}
    \caption{ACCESS-ESM1-5 carbon pools. Left: esm-ssp585-ssp126Lu relative to the 2005–2025 period. Right: difference between the esm-ssp585-ssp126Lu and esm-ssp585.}
...
%    \end{subfigure}
    ...
    \end{subfigure}
    \caption{Left: Changes in plant functional types for select gridpoints in esm-ssp585-ssp126Lu in ACCES-ESM1-5. Right: Distributions of maximum daily temperature for the last 20 years of esm-ssp585-ssp126 (green) and esm-ssp585 (yellow).}
...
\begin{itemize}
    ...
    \item No appreciable difference in climate variables at the global scale.
...
\begin{itemize}
   ...
   \item Forestation also opens the way for further development of forest management stategies to remove carbon dioxide, for example by using plantation biomass to generate energy, combined with carbon capture and storage.
\end{itemize}
