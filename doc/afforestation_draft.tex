...
The change in global surface air temperature in response atmospheric CO$_2$ concentrations and surface albedo is modest.
Most models show that the trend in the difference of the afforestation and reference simulations over the course of the century is not significant (Fig. \ref{fig:models_tas_trends}).
If there are trends in global temperature in response to forestation, the models largely dissagree on the sign of that change.
For example, NorESM5 shows a strong increase in global temperature of 0.3 $^{\circ}$C, suggesting that in this model the effects of decreased surface albedo dominates.
This contrasts to BCC-CSM2-MR, which shows a weak but significant decreasing trend in response to forestation.
...
NorESM5, is the only model that shows a significant increase in precipitation in the forestation scenario.
