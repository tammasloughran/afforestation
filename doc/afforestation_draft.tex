...
\begin{keypoints}
\item Forestation under a high emissions scenario is projected to generate a limited but stable carbon sink.
...
We analyze the results from seven CMIP6 models for a combined scenario with high fossil fuel emissions (from SSP5-8.5) and forest expansion (from SSP1-2.6).
...
The model intercomparison shows that xkm of forestation as a CO$_2$ removal strategy has limited impact on global climate under a high global warming scenario, despite generating a substantial cumulative carbon sink of 10--60 Pg C over the period 2015--2100.
...
We use seven model projections to estimate future climates in which forestation occurs under a high fossil fuel emission scenario.
...
Instead, the reduction in roughness transforming forest to short, smooth grass or cropland vegetation leads to less efficient transfer of heat from the surface into the atmosphere, which induces warming both in the annual mean and daily and seasonal warm extremes \cite{winckler_different_2019}\add{.}
...
This scenario assumes low population growth, low pressures on land-use,\add[TL]{ expansion of protected lands,} environmentally friendly changes in diets and increased agricultural efficiency and yields, which altogether drive the abandonment of agricultural lands.
...
These data are} translated \add[TL]{by each modeling group} to changes in the fractional coverage of \change[TL]{model}{either specific} plant functional types (PFTs) \add[TL]{or more general land-use and land-cover types if PFT competition dynamics are simulated}\remove[TL]{by each modeling group} \cite <for example>{di_vittorio_land_2014}.
...
The SSP1-2.6 land-use change is typically characterized by forestation, while the SSP5-8.5 has a mixture of deforestation and forestation.
...
Furthermore, we acknowledge that the LUH2 SSP1-2.6 scenario underestimates the tree cover that was originally dictated by IMAGE (the \change[TL]{integrated assessment model}{IAM} that produces the SSP1-2.6 scenario).
...
Therefore, we consider \textit{esm-ssp585-ssp126Lu} as having low forestation and would be representative of the lower end of the future mitigation potential from forestation.
...
For the global \add[TL]{mean} analysis across \remove[TL]{all}CMIP6 models,  \change[TL]{w}{we analyze global forest cover fractions, terrestrial and oceanic carbon, surface air temperature and precipitation. W}e calculate trends in the difference of global mean temperatures and precipitation to examine the change in temperature as forests expand.
...
The unique representations of forest cover ar\remove{r}ise from a variety of sources.
...
\add[TL]{Several show large-scale forest loss in some regions.
...
The \citeA{sonntag_quantifying_2018} study consists of only a single model that may have an incomplete representation of the real world.
...
\add[TL]{The scenario difference in treeFrac for each region in shown in Figure {\ref{fig:accesss_regional_cland}}a and demonstrates the extent of forestation that occurs in these regions.
...
Figure \ref{fig:accesss_regional_cland}\add[TL]{b} shows the change in total land carbon for each of the regions outlined in Figure \ref{fig:land_use}c.
...
This forestation scenario features high fossil fuel emissions, a much warmer climate, forestation and agricultural abandonment.
...
\begin{acronyms}
    ...
    \acro{SSP1-2.6} Low fossil fuel emissions scenario with forestation
