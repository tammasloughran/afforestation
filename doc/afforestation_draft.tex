...
\begin{keypoints}
\item \change[TL]{Moderate }{F}orestation under a high emissions scenario is projected to generate a limited but stable carbon sink.
...
We analyze the results from seven CMIP6 models for a combined scenario with high fossil fuel emissions (from SSP5-8.5) and \remove[TL]{moderate }forest expansion (from SSP1-2.6) \add[TL]{ which has $\sim$4.8\% increase in global total forest cover relative to 2015}.
...
The model intercomparison shows that \remove[TL]{moderate }forestation as a CO$_2$ removal strategy has limited impact on global climate under a high global warming scenario, despite generating a substantial cumulative carbon sink of 10--60 Pg C over the period 2015--2100.
...
We use seven model projections to estimate future climates in which \remove[TL]{moderate }forestation occurs under a high fossil fuel emission scenario.
...
Model results are confirmed by observation-based estimates (which by way of their setup capture only local effects; \citeA{alkama_biophysical_2016,bright_local_2017} and see \citeA{pongratz_land_2021} for a review of the climatic effects of forest cover changes from local to global scale).
...
The reference simulation we use to compare to \change[TL]{\textit{esm-ssp585-ssp126Lu}}{the forestation scenario} is the \textit{esm-ssp585} from the Coupled Climate-Carbon Cycle Model Intercomparison Project (C4MIP) \cite{jones_c4mip_2016}.
...
The forestation scenario analyzed here is the Land Use Model Intercomparison Project's \textit{esm-ssp585-ssp126Lu}, spanning the years 2015--2100 \cite{lawrence_land_2016}. \add[TL]{Compared to SSP5-8.5, this scenario has has $\sim$4.8\% increase in forest cover}.
...
The land-use change data used in all CMIP6 experiments are the Land-use Harmonization data set version 2 \cite<LUH2;>{hurtt_harmonization_2020}, which for future scenarios is derived from integrated assessment models (IAMs).
...
These data are translated by each modeling group to changes in the fractional coverage of either specific plant functional types (PFTs) or more general land-use and land-cover types if PFT competition dynamics are simulated \cite<for example>{di_vittorio_land_2014}.
...
The difference in forest area between the two scenarios (Figure S1) is about 3 million km$^2$ (\add[TL]{which is} an area about the size of India\add[TL]{and $\sim$4.8\% increase in global total forest cover}), about 50\% of which is avoided deforestation in the SSP5-8.5 scenario.
...
Furthermore, we acknowledge that the LUH2 SSP1-2.6 scenario underestimates the tree cover that was originally dictated by IMAGE (the IAM that produces the SSP1-2.6 scenario).
...
\begin{sidewaystable}
    ...
    \begin{tabular}{@{}lllllllp{0.07\textwidth}lp{0.15\textwidth}l@{}}
        \hline
Model         & Fire & N   & P   & PFT dyn. & Demography & PFTs & Natural PFTs & Resolution    & LSM Reference                                                     & ESM Reference                       \\ \hline
...
This forestation scenario features high fossil fuel emissions, a much warmer climate and \remove[TL]{moderate }forestation and agricultural abandonment.
...
\begin{acronyms}
    ...
    \acro{SSP1-2.6} Low fossil fuel emissions scenario with \remove[TL]{moderate }forestation
