...
In a modelling study of deforestation, \cite{boysen_global_2020} show a cooling of -0.22$\pm$0.2 \textcelsius{} among 9 climate models in idealized deforestation simulations with constant atmsospheric CO$_2$ concentration.
...
Some other experimental designs are more idealized and represent a more extreme deployment of forestation, where a huge portion of non-forested lands are forested.
...
While this is unfeasible in the real world, it allows for diagnosis of local and non-local effects of forestation, demonstrating that in those models the local biogeophysical effects forestation produce a cooling in the tropics, while the non-local effects are a large scale warming.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Changes in tree fraction from the Land-use Harmonization 2 SSP1-2.6 (top) and SSP5-8.5 (bottom) scenario between the year 2100 and 2015. Boxes demark the areas used in the ACCESS-ESM1-5 regional analysis. The black crosses marl the locations of large forestation changes that are used for daily temperature distributions.}
    \label{fig:land_use}
\end{figure}
...
CanESM5 has differing ensemble initialisation methods for \textit{esm-ssp585} and \textit{esm-ssp585-ssp126Lu}.
...
The data used in this study are avaialable from the Earth System Grid Federation.
...
For the global analsysis accross all models, we calculate trends in the difference of global mean temperatures and precipitation to examine the change in temperature as forests expand.
...
For the gridpoint maximum temperature frequency histograms, the difference in the temperature distribution in the two simulations in response to forestation is tested using the 2-sample Kolmogorov-Smirnov test for the equality of distributions at the 5\% level.

Lastly, to examine the relationship of forestation on climate, correlations were done on ensemble mean surface air temperature and tree fraction using the Spearman rank correlation and the significance was tested at the 5\% level \parencite{kokoska2000crc}.
...
CanESM5, MPI-ESM and UKESM1-0-LL also feature substantial Amazonian die-back in both scenarios, typically driven by localised drying.
Secondly, MPI-ESM1-2-LR has large amounts of tree cover increase in semi-arid regions in Africa and Australia. \footnote{I've asked some MPI people why this occurs but still waiting a response.}
Thirdly, the UKESM1-0-LL \textit{esm-ssp585-ssp126Lu} scenario is known to have enhanced CO$_2$ fertilisation and warming in the mid- to high-latitudes (resulting in increased tree cover fractions in Fig. S\ref{fig:tree_area_maps_ssp126Lu}) and decreased tree cover fraction in tropical South America and southeast Asia, driven by a combination of land use change and regional drying trends.
...
By mid-century, crops reach a minimum of 2.74 million km² less than in 2015, before rising again in the latter half of the century (Fig. S\ref{fig:ACCESS_land_use}).
...
The diversity of increased tree cover fraction among the models corresponds to a range of carbon drawdown potentials into the terestrial system.
...
For vegetation carbon, the models either maintain a draw-down potential of $\sim$20--50PgC by 2100 (ACCESS-ESM1-5, UKESM1-0-LL, MPI-ESM1-2-LR and CESM2), or the vegetation gains in the middle of the century have been lost by 2100 (CanESM5 and MIROC-ES2L).
...
Soil carbon shows varied response to forestation, but most models show that carbon that accumulates in litter and soil pools which is a long term sink over the duration of the 21st century.
...
On the other hand, ACCESS-ESM1-5 and UKESM1-0-LL show decreases in soil carbon in response to forestation. \footnote{Still working on the why of this.}
...
The ACCESS-ESM1-5 ensemble range demonstrates that atmospheric CO$_2$ is strongly sensitive to internal climate variability, encompasing 60\% of the multi model range.
With concentrations projected to increase from 400 ppm 1050 ppm \ref{fig:models_co2_absolute} under SSP5-8.5, this represents only a fraction of the required decrease to fully mitigate greenhouse gas forcing.

\subsubsection{Climate response}

Fig. \ref{fig:models_tas_trends} shows the difference in global mean surface air temperature showing that the trends in temperature over the course of the century is not significant in all models.
...
The temporal variance of temperature also shows unique behaviour among the models.
For example, MIROC-ES2L features large decadal scale oscillations in global mean teperature in both the forestation scenario and the reference scenario (Fig. S\ref{fig:models_absolute}), which causes large oscillations in the difference as they drift out of phase in the latter half of the century.
...
The region with the largest change in land carbon content is Central Africa (Fig. \ref{fig:accesss_regional_cland}e), however this difference is due to avoided deforestation that occurs in the reference simulation, rather than due to new forest growth in the forestation experiment (Fig. S\ref{fig:ACCESS_land_cover}a).
...
Some regions show decreases in sirface air temperature in response to increasing tree cover.
