...
\cite{hong_impacts_2022} also show that under a future deforestation scenario this cooling effect might reduce the incident of hot extremes by 5.5--0.9\%.
...
Furthermore, the DECK specifies corresponding simulations (\textit{esm-piControl} and \textit{esm-hist}) that are run in ``Earth system model" mode (i.e. there is a fully interactive dynamical carbon cycle) with fossil fuel and industrial greenhouse gas emission forcings during the historical period.
...
The former uses r1i1p1f1 and the latter uses r1i1p2f1, which features recent bugfixes in the model.
...
To interepret the difference in tree cover response of the models, it is helpfull to be aware of some of the known climate and dynamic features of each model.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Differences for cLand, cVeg and cLitter+cSoil between the \textit{esm-ssp585-ssp126Lu} and the \textit{esm-ssp585} scenario for 6 CMIP6 models. ACCESS-ESM is plotted as the ensemble mean and the blue shading indicates the ACCESS-ESM1-5 ensemble range.}
...
Figure \ref{fig:model_cpools}, demonstrates the change in the model terrestrial carbon pools due to forestation.
...
Soil carbon shows varied response to forestation, but most models show that carbon accumulates in litter and carbon pools which is a long term sink.
...
On the other hand, ACCESS-ESM1-5 and UKESM1-0-LL show decreases in soil carbon. \footnote{Still working on the why of this.}
...
%BCC-CSM2-MR shows a weak but significant decreasing temperature trend in response to forestation.
