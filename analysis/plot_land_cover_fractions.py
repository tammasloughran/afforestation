...
if __name__ != 'analysis.plot_afforestation':
    ...
    from constants import FRAC_VARIABLES
else:
    from analysis.cdo_calc_load import cdo_cover_area_load
    from analysis.cmip_files import get_filename
    from analysis.constants import FRAC_VARIABLES

COLORS = {'treeFrac':'green',
        'cropFrac': 'gold',
        'shrubFrac': 'olive',
        'grassFrac': 'lime'}
LABELS = {
...
def make_land_cover_plot():
    ...
    for var in FRAC_VARIABLES[table]:
        ...
        plt.plot(years, aff_areas[var]/10**12, color=COLORS[var], label=LABELS['AFF'][var])
        frac_file = get_filename('C4MIP', 'esm-ssp585', '1', table, var)
        ssp585_areas[var] = cdo_cover_area_load(frac_file[0], var)
        plt.plot(years, ssp585_areas[var]/10**12, color=COLORS[var], linestyle='dashed',
